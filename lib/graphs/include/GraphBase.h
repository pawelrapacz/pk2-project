...
namespace graphs
{
    ...
    public:
        ...
        bool constains(const key_type& vertex) const { return vertices_.contains(vertex); }

        bool empty() const noexcept { return vertices_.empty(); }

        void clear() noexcept { vertices_.clear(); }

        vertices_map& data() noexcept { return vertices_; }

        const vertices_map& data() const noexcept { return vertices_; }

        value_type& operator[](const key_type& vertex) {
            static_assert(graph_traits::is_not_void_v<value_type>,
                          "To use this function, value_type cannot be void.");
        ...
        bool insertVertex(const key_type& newVertex) {
            static_assert(std::is_default_constructible_v<value_type> || std::is_void_v<value_type>,
                          "To use this function, value_type has to be default constructible.");
            return vertices_.try_emplace(newVertex).second;
        }

        template<typename V = value_type>
        graph_traits::enable_if_not_void_t<V, bool> insertVertex(const key_type& newVertex,
                                                                 V value) {
            return vertices_
                .insert({
                    newVertex, {value, edge_list()}
            })
                .second;
        }
